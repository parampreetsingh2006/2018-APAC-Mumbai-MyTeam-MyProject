...
getEachSentenceIntent = function(session) {
    ...
    //momBody += "<br /><b>Updates for " + userName + " </b><br />";
...
bot.dialog('HelpDialog',
    (session) => {
        session.send('Ok can you let me know on which topic you need help on?');
        session.endDialog();
    }
).triggerAction({
    matches: 'Help'
...
bot.dialog('CancelDialog',
    (session) => {
        session.send('Cancellation done!');
        session.endDialog();
    }
).triggerAction({
    matches: 'Cancel'
...
bot.dialog('EmailDialog',
    (session, results) => {
        ...
        emailHandler.sendEmail(session);
    }
).triggerAction({
    matches: 'MOM-Email'
})
...
testLuis = function (message, session) {
   ...
   console.log("calling kluis for " + message + " linesCount " + linesCount);
    ...
    request(options, function (error, response, body) {
        ...
        linesCount--;
        if(linesCount == 0) {
            luisResponseParseIndex = -1;
            parseLuisResponse(session);
            console.log("############################");
            console.log(momBody);
        }
...
var momSubject = "Minutes of Meeting";
var momBody = "<h1>Minutes of Meeting</h1> <br /> Updates for user : <br> Scrum-13: Completed <br> Scrum-14:In progress";

parseLuisResponse = function(session) {
     let index = ++luisResponseParseIndex;
       ...
       if(index < luisResponse.length) {
        ...
        if(luisResponse[index].topIntent == "Jira-Status") {
            let ent = luisResponse[index].entities.find(entity => entity.type == 'Jira-Id');
            let jiraId = "scrum-13";
            if(ent) {
                jiraId= ent.entity.split(' ').join("");
            }
            ent = luisResponse[index].entities.find(entity => entity.type == 'Jira-Status')
            let jiraStatus = "done";
            if(ent) {
                jiraStatus= ent.entity.split(' ').join("");
            }
           jiraHandler.statusJiraLuisResponse(jiraId, session, jiraStatus);
        ...
        session.send("Scrum done !!!");
        session.beginDialog('needAnyOtherHelp');
       }
       
    
};

bot.dialog('needAnyOtherHelp',[
    function (session) {
      builder.Prompts.text(session, "Do you need any other help?");    
    },
    function (session, results) {
        session.send("Email sent !");
        emailHandler.sendEmail(session,momBody);
        session.endDialog();
...
bot.dialog('confirmJiraUpdate2',[
    ...
    function (session, results) {
        if(results.response) {
            jiraHandler.updateStatusLuisResponse(argsCurrent.jiraId,argsCurrent.jiraStatus,results, session);
        } else {
