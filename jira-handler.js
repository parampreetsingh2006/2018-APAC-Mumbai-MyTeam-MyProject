        	   ...
        	   "body": [
		          {
                      ...
                      
                     {
                      ...
                      {
                        "type": "Image",
                        "url": "https://image.ibb.co/iHFBee/jira_status.png",
                        "size": "large",
                        "style": "person"
                      },
                      {
...
var statusJiraLuisResponse = function(jiraId, session, jiraStatus) {
 console.log("url is " + jiraHost + '/rest/api/2/issue/'+jiraId);
  var options = { method: 'GET',
    url: jiraHost + '/rest/api/2/issue/'+jiraId,
    headers:
     { 'postman-token': 'fc7df2de-54ed-3cef-3901-79b2271a9280',
  ...
  request(options, function (error, response, body) {
    ...
    console.log("======================================"+JSON.stringify(body));
    console.log("JIRA STATUS IS: "  + body.fields.status.name);
    let currentJiraStatus = body.fields.status.name;
    if(jiraStatus.toUpperCase() != currentJiraStatus.toUpperCase()) {
      let args = {};
  ...
  var updateJiraStatusLuisResponse = function(session, jiraId, transitionId) {
    console.log("update jira status transtion id: " + transitionId + " Jira id: " + jiraId);
    var options = { method: 'POST',
      url: jiraHost + '/rest/api/2/issue/'+jiraId + '/transitions',
      headers:
    ...
    request(options, function (error, response, body) {
      if (error) throw new Error(error);
      session.send("Jira status updated !!");
      session.endDialog();
      parseLuisResponse(session);
    ...
    var updateStatusLuisResponse = function(jiraId, status, response, session) {
      ...
      request(options, function (error, response, body) {
        ...
        if(!body || !body.transitions) {
         ...
         var transitionId = body.transitions.find(transition =>transition.name.toUpperCase() === status.toUpperCase());
          ...
          console.log("=RESPONSE-" + response);
          console.log("=transition id-" + transitionId.id);
          updateJiraStatusLuisResponse(session, jiraId, transitionId.id);
      });
    };
...
module.exports.statusJiraLuisResponse = statusJiraLuisResponse;
module.exports.updateJiraStatusLuisResponse = updateJiraStatusLuisResponse;
module.exports.updateStatusLuisResponse = updateStatusLuisResponse;
